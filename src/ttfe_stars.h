...
static inline void generate_starfield(EntityPool* pool, int count, float min_r, float max_r) {
    pool_clear(pool);
...
static inline void render_starfield(const EntityPool* pool, VertexArray* va, float light_phase) {
    va_clear(va);
...
static inline void render_pink_lights(const EntityPool* pool, VertexArray* va, Vec3 cam_right, Vec3 cam_up, float light_phase) {
    va_clear(va);
